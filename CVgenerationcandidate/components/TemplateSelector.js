...
import { templates, getTemplatesByCategory, getFreeTemplates, getPremiumTemplates, searchTemplates, getFeaturedTemplates } from '../data/templates';
import { Check, Eye, Star, Filter, Search, Grid, List, Crown, Zap, Award, TrendingUp, Sparkles, Target, Users, Briefcase } from 'lucide-react';
...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'name' | 'category' | 'difficulty' | 'popularity'>('name');
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return filtered;
  }, [searchTerm, selectedCategory, selectedIndustry, selectedDifficulty, showPremiumOnly, showFeatured, sortBy]);
    ...
    return (
      <div
        className={`bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer transform hover:-translate-y-1 ${
          selectedTemplate?.id === template.id ? 'ring-4 ring-blue-500 ring-opacity-50' : ''
        }`}
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {template.industry.slice(0, 3).map(industry => (
              <span key={industry} className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-medium">
                {industry}
              </span>
            ))}
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {template.features.slice(0, 3).map(feature => (
              <span key={feature} className="bg-green-50 text-green-700 px-2 py-1 rounded text-xs flex items-center gap-1">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-gray-500 ml-2">{template.fonts.heading}</span>
            </div>
            ...
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium border ${getDifficultyColor(template.difficulty)}`}>
                    {template.difficulty}
                  </span>
                  {template.premium && (
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="text-center mb-8">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Select from our collection of {templates.length} professionally designed templates. 
              Each template is crafted to highlight your unique skills and experience across {categories.length - 1} different categories.
            </p>
          </div>
      ...
      <div className="bg-white border-t">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-6 text-center">
            ...
            <div>
              <div className="text-3xl font-bold text-green-600">{getFreeTemplates().length}</div>
              <div className="text-gray-600">Free Templates</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-yellow-600">{getPremiumTemplates().length}</div>
            ...
            <div>
              <div className="text-3xl font-bold text-indigo-600">{industries.length - 1}</div>
              <div className="text-gray-600">Industries</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-red-600">{getFeaturedTemplates().length}</div>
